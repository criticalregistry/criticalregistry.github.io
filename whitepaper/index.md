...
Global digital infrastructure depends on a layer of open source software components that few organisations have mapped and almost none have funded. Connected products across automotive, medical devices, industrial automation, and rail run on this infrastructure. These components are maintained, in many cases, by individual people working without institutional support, in programming languages with shrinking developer pipelines, against a backdrop of nation-state actors who have identified maintainer burnout as a systematic attack surface.
...
The numbers are not small. Twenty billion installations of a single networking library with one primary maintainer since 1996. Most of the world's encrypted internet traffic secured by a library that had a single full-time developer when its most critical vulnerability was discovered in 2014. A nation-state attack in 2024 that came within days of shipping a backdoor into SSH on most Linux distributions. The attack used no technical exploit. It relied on approximately three years of patient relationship-building with one exhausted volunteer maintainer.

Each incident has had a larger blast radius than the last. In 2025 and early 2026, the pattern continued and accelerated. A widely-used XML parsing library briefly lost its only maintainer, declaring the project "more or less unmaintained for now." A Kubernetes component used in approximately half of all cloud-native environments was formally retired after its volunteer team burned out. AI-assisted vulnerability discovery tools identified 271 vulnerabilities in a single browser codebase in one evaluation pass, a number that exceeds the entire previous year's high-severity findings for the same project. The next major incident will be in hardware: in automotive firmware, in medical devices, in industrial control systems where patching is measured in years, not days. The questions are not just "can we patch" but "can we still get a patch at all, and do we even know what we're running."

This paper argues that the problem has a correctable structure, but the entity needed to correct it does not yet exist. It proposes what that entity looks like, how it operates, and why it is a business rather than a charity.
...
Consider a few examples. None of them are edge cases. They are the infrastructure on which products run today:

**libcurl** is the software library responsible for data transfer across networks. It is present in an estimated twenty billion installations: in cars from almost every major manufacturer, in medical devices, in game consoles, in nuclear fusion experiments, in every major operating system, in the firmware of industrial controllers. It has been maintained since 1996 primarily by Daniel Stenberg, a Swedish engineer employed full-time by wolfSSL since 2019 to work on curl. curl has hundreds of contributors, but the contributor community cannot sustain the project without Stenberg. The wolfSSL arrangement, where a company employs him to work on curl and offers commercial curl support, is the model this paper argues every critical project deserves: sustained, commercial, transparent funding tied directly to the work. The residual risk is not absence of funding. It is that the entire project's continuity still depends on one person remaining available, engaged, and employed in this specific arrangement. The goal is not to monitor that risk indefinitely. It is to build on the wolfSSL model until the bus factor no longer sits at one.

**OpenSSL** provides the cryptographic layer for most encrypted internet traffic, covering authentication, data-in-transit protection, and certificate validation across enterprise, government, and consumer infrastructure. When the Heartbleed vulnerability was publicly disclosed in April 2014, OpenSSL had one full-time developer. The bug had been present in the codebase since 2012. For two years, any attacker could have been reading memory from any vulnerable server without leaving a trace. The estimated remediation cost was approximately $500 million, considering only the cost of certificate revocation and reissuance. Actual costs, including human resources, emergency audit programmes, and the irretrievable exposure of credentials and private keys, were substantially higher.

**Samba** implements the SMB network file-sharing protocol, providing the interoperability layer between Windows and Linux/Unix systems across enterprise, government, and industrial environments. It underpins file sharing, printer sharing, and Active Directory integration in organisations ranging from law firms to railway operators to defence contractors. Its active committer base numbers approximately ten to fifteen people, per its own team page. The core codebase is written in C. It is funded episodically, most recently by the German Sovereign Tech Fund at approximately €688,000, managed by SerNet over 18 months.

**zlib, libjpeg, libpng, expat, FreeType, BIND, and glibc** follow the same pattern. These components handle compression, image parsing, DNS resolution, text rendering, and the fundamental C standard library. They are embedded in billions of devices and depend on teams of five to fifteen people, many of them volunteers, some of them operating without any institutional funding at all.
...
In proprietary software, the vendor carries operational accountability. Microsoft disclaims warranties in its licence terms, as does every software vendor. But Microsoft also maintains a dedicated security response team, publishes a patch cadence, defines support lifecycles, and faces reputational and commercial consequences for inaction. The risk is priced into the licence fee, the support contract, and ultimately the vendor's share price. There is an escalation path. There is someone to call.
...
The legal landscape is shifting, and shifting fast. The EU Cyber Resilience Act entered into force in December 2024, with vulnerability reporting obligations taking effect in September 2026 and full enforcement from December 2027. The regulation requires manufacturers of products with digital elements to know what components are in their products, to maintain them securely across the product lifecycle, and to report actively exploited vulnerabilities within 24 hours of becoming aware. The FDA's updated software guidance creates similar obligations in the medical device sector. SEC cyber disclosure rules in the United States require public companies to report material cybersecurity incidents and describe their risk management processes.

Each of these regulatory instruments creates the same practical requirement: organisations need to know what software is in their products. Many of them currently do not. And the act of discovering what is in their products will, for many organisations, surface dependencies on components that are materially unmaintained.

Insurance underwriters are arriving at the same place from a different direction. Cyber insurance renewal questionnaires are increasingly asking about software bill of materials practices, open source dependency management, and patch management for third-party components. For organisations that cannot answer these questions, the consequences are higher premiums, coverage exclusions, or outright refusal to underwrite. The risk is being repriced. The question is whether manufacturers will price it themselves or wait for insurers and regulators to do it for them.
...
In early 2024, a contributor named "Jia Tan" who had spent approximately three years building trust with the XZ Utils community shipped a sophisticated, carefully obfuscated backdoor into the library. He had provided quality patches, offered code review, and gradually earned commit access. The backdoor, if it had reached production in major Linux distributions, would have compromised SSH authentication on most Linux servers in the world. It was discovered almost by accident, by a Microsoft engineer who noticed anomalous CPU usage in a benchmarking test.

The technical sophistication of the attack was real. The social engineering sophistication was greater. The attacker did not exploit a software vulnerability. They exploited a human one: a burned-out, unsupported maintainer, isolated in a role no one else would take, who was grateful for help. The incident confirmed what security researchers had been saying for years: nation-state actors have identified the maintainer burnout problem and are treating it as an attack surface.

**libxml2 (2025)** offered a preview of what an unmanaged availability failure looks like. libxml2 is the XML parsing library used by virtually every Linux distribution, every major browser's HTML engine, and embedded across billions of devices. In mid-2025, its sole maintainer announced he would no longer honour security embargoes. That is the standard practice of holding vulnerability details private while a patch is prepared and distributed. The unpaid coordination burden had become unsustainable. In his own words, the project should be treated as software "maintained by a single volunteer, badly tested, written in a memory-unsafe language and full of security bugs." By September 2025 he had stepped down entirely, declaring the project "more or less unmaintained for now." New maintainers eventually stepped forward, but for a period, a component embedded in billions of devices had no active maintainer. The near-miss received brief coverage and was then forgotten.
...
Both organisations have framed this as a net positive for defenders: if AI can find vulnerabilities at this scale and speed, and AI can also help generate and validate fixes at comparable speed, the cycle compresses and defenders stay ahead. For well-resourced IT environments with automated deployment pipelines, this argument is plausible and may prove correct.
...
The Linux kernel CVE flood compounds this further. Since the kernel project became a CNA (Common Numbering Authority) it has adopted an approach of issuing CVEs for almost every potential flaw, on the argument that any kernel bug could be leveraged as a vulnerability. The numbers illustrate the scale: 2020 saw 120 kernel CVEs for the entire year; 2024 logged 3,529, a nearly tenfold increase in four years. In the first 16 days of 2025 alone, 134 new kernel CVEs appeared, a rate of 8 to 9 per day, counting only kernel vulnerabilities with no userspace, libraries, or anything else included. Downstream distributions that depend on cherry-picking critical CVEs from the kernel stream are now working against a signal-to-noise ratio that makes prioritisation nearly impossible. The copy.fail vulnerability (CVE-2026-31431), publicly disclosed on 29 April 2026, illustrated the failure mode precisely: a proven-exploitable local privilege escalation flaw received a CVE, was buried in the flood, and every major Linux distribution was blindsided because the standard coordinated disclosure process was bypassed. The coordination infrastructure did not fail because of malice. It failed because the volume exceeded the capacity of the people responsible for running it.
...
The next major incident will be in hardware. Automotive firmware is compressing its patch timelines through OTA updates, but only where that infrastructure exists and is mature. A patch to a medical device still requires an FDA submission, regardless of how quickly it was written. Railway interlocking recertification takes weeks to months even for minor changes. That timeline is the floor, not the ceiling.

### 1.4 The OT Dimension: Security and Availability at Long Timescales

The incidents described above were serious. They affected enterprise IT systems, web servers, cloud infrastructure. They were costly, disruptive, and in some cases irreversible. All were patchable, in principle, within a reasonable timeframe. The affected systems could be taken offline, updated, brought back online. In the worst cases, incident response teams were engaged, systems were rebuilt, operations were disrupted for days or weeks.
...
The patching model that makes IT security tractable, the discover-test-deploy-verify cycle, does not apply in OT. A railway signalling system may be certified to SIL4 (the highest safety integrity level under the CENELEC railway standards, comprising EN 50126 for RAMS, EN 50716:2023 for software, and EN 50129 for safety-related electronic systems, derived from the generic IEC 61508 framework) and remain in service for twenty to thirty years. A software update to a SIL4-certified system requires re-certification: a process that takes weeks to months depending on the scope of change, and carries real cost. OT systems therefore operate on component versions that IT teams would consider dangerously obsolete. They are correct to do so. The alternative is either non-certification or a recertification process for every patch cycle.
...
These compensating controls are the right answer for the security dimension. Hardening the IT/OT barrier, through network segmentation, traffic monitoring, and unidirectional data flows where feasible, is the established and correct approach to managing exposure when a component cannot be patched. A vulnerability in a library embedded in a SIL4-certified interlocking is a security and network architecture problem, not a safety certification problem. Safety engineers in this domain already know not to trust third-party library code. SIL4 development methodology requires verifying or replacing components precisely because you cannot assume anything external is trustworthy. That discipline is well-established.
...
Maintaining a proprietary fork is the theoretical alternative. In practice it requires the manufacturer to take on indefinite responsibility for a codebase outside their core competency, fund recertification every time they patch it, and replace institutional knowledge that took decades to accumulate. The economics reliably favour funding the original project.
...
The **German Sovereign Tech Fund**, established in 2022, provides government grants to critical open source projects. Its model is correct in principle: direct, multi-year funding for specific projects based on their criticality to digital infrastructure. The Samba project's €688,000 funding round is a representative example. The limitations are scale (annual budgets of €13 million in 2022 and €22 million in 2023, large for a single government programme but insufficient relative to the global dependency), geography (Germany and to some extent the EU), and duration (grants rather than sustainable ongoing funding structures). It cannot compel action, it cannot move at private-sector speed, and it cannot address the availability and continuity risk in OT-dependent industries where the stakes are highest.

The **Core Infrastructure Initiative (CII)**, now absorbed into OpenSSF, was established in direct response to Heartbleed. It funded projects including OpenSSL, OpenSSH, and the Linux kernel. It made a real difference. It was also reactive: it addressed a specific crisis rather than building a systematic capability to identify and prevent the next one.

These initiatives share a common failure mode: they operate within the engineering community, on engineering timescales, speaking engineering language. The people who control the capital required to fix the problem structurally, at manufacturers, insurers, and investment firms, are not in the room.
...
The open source community has engineers. Some of them extraordinary: people who have maintained intricate, widely deployed systems for decades with minimal support, at a level of quality that would be difficult to replicate under any funding model. The problem is not a shortage of engineering talent.
...
**Systems architects** who can look at a twenty-year-old codebase and design the path forward: not by accumulating patches on existing foundations, but by thinking clearly about what needs to be preserved, what can be discarded, and what needs to be rebuilt.
...
**Governance structures** that provide accountability without bureaucracy, able to accept funding, make decisions, and execute without requiring consensus from a mailing list of several hundred interested parties.

SQLite is the most frequently cited example of what is possible. It succeeds not because it has more money than comparable projects, but because it has more operational structure: a small, professional team, a clearly defined scope, thorough documentation, and a governance model that allows fast decisions. Its creators and independent researchers describe it as the most widely deployed database engine in the world, present in every iOS and Android device, every major web browser, and billions of other installations. The constraint was never money alone. It was structure applied to engineering effort.

---

## Part 3: The Proposed Solution

The three capabilities described below, the map, the certification framework, and the intervention model, are not a simultaneous build. They are a sequence. The map comes first. It is the only deliverable that can be built, published, and validated before anything else exists. Certification follows once the map has users, critics, and enough real-world data to make assessments credible. Intervention consulting is the third phase, and it requires something the first two phases do not: the capacity for extended client-facing engagements, which is a different organisational commitment entirely. This paper describes all three because they belong to the same logical architecture. But the entity that builds this starts with the map and nothing else. Everything else unfolds from demonstrated credibility, not from a prospectus.
...
- **Maintainer continuity:** bus factor above a defined threshold; succession planning documented; core knowledge not resident in a single person's head
...
The certification framework is not purely a carrot. A component's absence from the certified list, or its failure to maintain certification, is itself information that the map makes visible. The combination of public risk assessment and certification status creates a complete risk picture: this component is critical, it is not certified, and here is why. Where a manufacturer has sponsored the certification of a specific component, that sponsorship is disclosed on the entry. The assessment outcome is independent of it.

One implication of this framework deserves explicit acknowledgment. curl, the paper's lead example of a well-funded project, would currently be designated At Risk under the bus factor criterion. The wolfSSL arrangement solves the funding problem. It does not solve the succession problem. One person still holds the institutional knowledge, the commit rights, and the relationships that keep the project functioning. That is precisely the risk this paper describes. An At Risk designation for curl is not a failure of the framework. It is the framework working correctly: naming the residual risk that remains even after the funding model has been addressed, and creating the pressure to resolve it. The goal is not to certify the current state. It is to drive the changes that produce a certifiable state.
...
The trust challenge is real. An entity that rates the sustainability of open source components and also advises on how to improve them will face questions about whether the ratings serve the advice business. The answer is not structural separation. That would break the feedback loop that makes the model work. A rater who designates a project At Risk and then helps fix it is doing the job end to end. Forced separation produces a rater who walks away after designating At Risk, which serves no one. The safeguard is disclosure. Any project with an active or recently completed consulting engagement is rated with that fact visible in the entry, flagged for all users of the data for the duration of the engagement and for twelve months after its conclusion. Buyers see the flag and apply their own judgment. That is more transparent than most financial products, and it is sufficient.

This is a solvable problem. Rating agencies operate under similar structural tensions: they assess the creditworthiness of issuers who pay for the ratings. The model has well-documented failure modes, but it has also generated durable, valuable institutions. The design principle that matters is that the data product's value depends on its perceived independence. Disclosure protects that independence. Concealment destroys it.
...
**Data subscriptions** are the core and highest-leverage revenue stream. The dependency map, continuously maintained and authoritative, is valuable to cyber insurance underwriters who need it to price risk, to private equity and M&A due diligence teams who need it to assess the software supply chain risk of acquisition targets, and to compliance teams at manufacturers who need it to evidence their regulatory obligations under the CRA and equivalent frameworks. These buyers have high willingness to pay for accurate, maintained risk data; the alternative is building the capability internally at considerably higher cost. Subscription revenue is recurring and scales with the breadth and depth of the map rather than with the number of consulting engagements.

**Certification fees** are paid primarily by the manufacturers whose compliance documentation benefits from referencing certified components. Open source projects rarely have the budget to seek certification themselves; the economic incentive sits with the organisations depending on them. A manufacturer who can list certified components in a CRA compliance filing, an FDA software review, or a cyber insurance application has a direct financial reason to fund that certification. Annual renewal creates recurring revenue. As the mark gains regulatory and insurance acceptance, the fee becomes a line item in a compliance budget rather than a discretionary expenditure. It becomes non-discretionary.

**Intervention consulting** is the highest-value and highest-margin engagement, but also the least scalable and the last to activate. It requires sustained client-facing capacity that the earlier phases do not. Each structured intervention assembles the team of architects, engineers, and programme managers described in Part 3 around a specific at-risk project, for a multi-year engagement at a price point that reflects the genuine scarcity of the required expertise. This revenue stream is not the primary growth driver. It is the proof of concept that demonstrates the model works and generates the institutional knowledge that makes future interventions cheaper and more effective.

**Training and tooling** helps organisations integrate SBOM practices, map their own dependencies, and implement the operational processes needed for CRA compliance. It is a volume play that opens doors into manufacturing organisations and creates the bottom-up awareness that drives the C-suite conversations.

### 4.3 The Moat

The defensible advantage in this business is not a technology. It is trust, data quality, and network effects.
...
**Build the MVP map.** Start with fifty components, the most widely deployed, the most clearly at-risk, the most verifiable in terms of maintainer data and dependency breadth. For each component, produce the full data set described in Part 3.1. Publish it. Make it citable. The first version does not need to be exhaustive; it needs to be accurate. One wrong data point that a manufacturer can challenge undermines the entire credibility model. Get the first fifty right.
...
**A note on funding the MVP.** The map costs money to build correctly. Fifty components assessed with the human intelligence layer described in Appendix A represents six to twelve months of focused effort before there is a product to sell. One credible bootstrap path runs through the manufacturers themselves. The EU CRA's vulnerability reporting obligations take effect in September 2026. Manufacturers who are not ready, and a large number are not, would pay for a structured pre-commercial dependency assessment today, in exchange for early data access and input rights on the certification framework. This produces revenue, validates the methodology under real conditions, and generates the three pilot C-suite conversations simultaneously. It is not the only path, but it is the fastest one that does not require external investment before the product exists.
...
**To open source maintainers:** The problem you are experiencing is structural, not personal. The burnout, the isolation, the sense that you are carrying a responsibility that nobody else will acknowledge: these are not individual failures. They are the predictable consequences of a funding and governance model that has never been fit for the criticality of the software it produces. This paper is not a compliance initiative directed at you. It is an argument directed at the organisations whose products depend on your work and who have not yet been made to face that dependency honestly. The map names you because accountability requires visibility. The goal is more wolfSSL arrangements, more Sovereign Tech Fund grants, more organisations treating the software they depend on as infrastructure worth funding. There is a path to sustainability that does not require martyrdom. It requires the same thing as any other critical infrastructure: the right organisational model, adequate resources, and someone with the standing to tell the people who depend on your work that they need to pay for it.

### 5.3 The Closing Argument

Rail safety certification exists because trains crashed until it did. The rail industry, one of the first to operate at scale with life-safety implications, developed safety integrity levels, formal methods, independent certification, and mandatory audit processes not because engineers thought it would be intellectually satisfying, but because regulators and courts demanded accountability after disasters.
...
**Named individual protocol.** The human intelligence layer names people. That requires a defined process, applied consistently. Before any entry containing a named individual is published, that person is notified directly with the specific data points about them that will appear in the map, and given a defined window to correct factual errors. This is not a consent requirement: public figures in their professional capacity, and their publicly documented professional activities, do not require consent for factual publication. It is a courtesy and a quality control measure. Corrections are applied before publication where the individual can demonstrate a factual error. Disputed judgments, for example a maintainer who contests the bus factor assessment, are noted in the entry alongside the assessment. Employer data is only included where it has been publicly disclosed by the individual themselves or their employer. Where employer information is not publicly available, it is omitted rather than inferred.
...
- Main obligations: secure-by-design requirements, SBOM for critical/important products, 24-hour reporting of actively exploited vulnerabilities, support period documentation
...
### Appendix D: Existing Initiatives
...
- Core knowledge documented: architectural decisions, edge case behaviours, test rationale
...
**Bus factor:** the minimum number of team members whose simultaneous loss would make a project unable to function. A bus factor of 1 means a single person's unavailability, from illness, burnout, or death, halts the project. High bus factor is a fundamental sustainability requirement.
