...
The numbers are not small. Twenty billion installations of a single networking library maintained by one person since 1996. Most of the world's encrypted internet traffic secured by a library that had a single full-time developer when its most critical vulnerability was discovered in 2014. A nation-state attack in 2024 that came within days of shipping a backdoor into SSH on most Linux distributions — not through a technical exploit, but through two years of patient relationship-building with one exhausted volunteer maintainer.

Each incident has had a larger blast radius than the last. The next one will be in hardware: in automotive firmware, in medical devices, in industrial control systems where patching is measured in years, not days — and where the questions are not just "can we patch" but "can we still get a patch at all, and do we even know what we're running."
...
**libcurl** is the software library responsible for data transfer across networks. It is present in an estimated twenty billion installations: in cars from almost every major manufacturer, in medical devices, in game consoles, in nuclear fusion experiments **[UNSOURCED — anecdotal/from Stenberg talks; remove or find primary source]**, in every major operating system, in the firmware of industrial controllers. It has been maintained since 1996 by one person — Daniel Stenberg, a Swedish engineer who works on it largely through his own company with limited commercial support. When a vulnerability is found in libcurl, the question of how quickly it can be patched and the severity of exposure across the installed base is, in material part, a question about what Daniel Stenberg has time for this week.
...
The pattern across these three incidents is consistent: small teams, inadequate support, increasing blast radius with each cycle. Heartbleed cost hundreds of millions. Log4Shell cost billions. XZ Utils nearly compromised a foundational authentication layer for a substantial fraction of internet infrastructure. The next incident will be in hardware. In automotive firmware, where a vulnerability in an embedded library requires a recall. In a medical device, where a patch requires an FDA submission. In a railway interlocking system, where "patch this week" is simply not a concept that the operational model accommodates.
...
The projects that demonstrate it is possible — SQLite, being the most frequently cited example — are successful not because they have more money, but because they have more operational structure. SQLite has a small, professional team, a clearly defined scope, comprehensive documentation, and a governance model that allows fast decisions. It has been embedded in more devices than any other database software in history **[UNSOURCED — widely claimed by SQLite creator Richard Hipp; find primary source or qualify as "claimed by its creators"]**, and it maintains an extraordinary record of reliability. The constraint was never money alone. It was structure applied to engineering effort.
...
Naming is not a shaming exercise. It is the minimum condition for accountability. "One maintainer" is an abstraction. "Daniel Stenberg, employed by his own company, maintaining libcurl since 1996, whose library is present in your vehicle infotainment firmware" is a named supplier relationship that any responsible supply chain manager would be required to assess.
...
*The author is developing a structured initiative to address this systematically. For collaboration, partnership enquiries, or further discussion, please contact: [placeholder email].*
...
- Attack method: social engineering over approximately two years
